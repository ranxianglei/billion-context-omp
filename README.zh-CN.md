...
| `transformMode` | 未设置（按 API 解析）或显式 `"provider"` / `"context"` | 压缩手术拦截位置。未设置：只要 wire 载荷可解析（anthropic-messages、ollama-chat、openai-completions——重建后的载荷会原地同步到原对象，宿主丢弃替换值也能送达，issue #79）就用 `provider`；不可解析的载荷（openai-responses / google / bedrock / cursor）用 `context`。显式设置总是生效。`provider` 变换 provider wire 载荷（请求局部，结构性免疫回灌环路）；`context` 改写 context 事件（旧版模式）。 |
