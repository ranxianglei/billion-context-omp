...
| `transformMode` | 未设置（按 API 解析）或显式 `"provider"` / `"context"` | 压缩手术拦截位置。未设置：仅当 host 会应用 wire 载荷替换时（anthropic-messages、ollama-chat）用 `provider`，其余 API 用 `context`（issue #79）。显式设置总是生效。`provider` 变换 provider wire 载荷（请求局部，结构性免疫回灌环路）；`context` 改写 context 事件（旧版模式）。 |
