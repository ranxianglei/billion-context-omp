...
14. **Provider-only transform (context mode removed, issue #52/#79)** — the surgery runs at `before_provider_request` on the WIRE payload, in CORE SPACE on the kernel codec (wire-fold.ts: payload→toCore→foldStreamCore/processTurn→coreToX — the same wire contract as the billion-context proxy). Request-local body → structurally no re-entry (the feedback-view loops #22/#47/01a0059b are impossible). The `context` event handler is a pure observer (logs + delivery warnings, returns undefined). Bodies the kernel cannot parse pass through (fail-open; the responses codec landed with kernel 0.0.32 — `responsesToCore`/`patchResponsesInput` rebuild the `input` layout-preserving, opaque items like `additional_tools` survive, string `input` stays a string). **Minimum host version: 17.3.8** (transform-mode.ts `MIN_HOST_VERSION` + `hostMeetsMinimum`) — enforced at `session_start` with a logWarn + ui.notify upgrade message. **Codec-viable APIs** (transform-mode.ts `CODEC_VIABLE_APIS`): `anthropic-messages`, `ollama-chat`, `openai-responses`, `openai-completions` — these serve a body the kernel codec recognizes AND the host applies the onPayload replacement. **Non-viable APIs** (bedrock Converse untyped content blocks, cursor gRPC AgentRunRequest, google, devin): the kernel has no codec path — `providerDeliveryWarning` fires a `nodelivery:` (bedrock/cursor: host drops the replacement fire-and-forget) or `nocodec:` (google/devin/unknown: no kernel codec) warning. **Upstream fix tracking: issue #83** — the dropped-replacement bug is fixed upstream (can1357/oh-my-pi PR #8717, merged; shipped in pi-ai 17.3.8); bedrock/cursor/google codecs are tracked as separate kernel work (acp-kernel issue #106). Live dual-extension hazard: `omp install` plugins + config extensions can run TWO instances — dev e2e must use `--no-extensions -e <dist>`.
15. **primeFold folds the WIRE projection (issue #64)** — the authoritative fold (and the span fingerprints the compress tool stores) run on the wire projection, a different space from the persisted session view: openai payloads carry the system prompt as the FIRST message (it takes `m00001`) and drop tool names on `role:"tool"` entries (the compress result is not ref-BLOCKED). Folding the raw session view at `session_start` lands refs/fingerprints in the wrong space, the span guard rejects every in-stream replay, and a resumed session shows "Blocks: none" until the first provider request. Fix: `primeFold` folds the format-matched wire mirror for EVERY session — openai shape (`viewToCoreStream`, system first) for openai/ollama payloads, anthropic shape (`viewToAnthropicCore`, system stays top-level/out of the fold space) for anthropic-messages, responses shape (`viewToResponsesCore`, system in the top-level `instructions` field, conversation in the `input` item array) for openai-responses — via the kernel codec, so the preview is in the same ref/fingerprint space as the live fold. The ACP system block is appended at prime time for the openai mirror because `before_agent_start` (which appends it on the wire) hasn't fired yet at `session_start`. The slot is still marked preview and re-folded authoritatively at the first live event.
