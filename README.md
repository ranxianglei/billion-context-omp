...
| `transformMode` | unset (resolved per API) or explicit `"provider"` / `"context"` — where the compression surgery intercepts. Unset: `provider` wherever the wire body is parseable (anthropic-messages, ollama-chat, openai-completions — the rebuilt payload is synced onto the original in place, so hosts that drop the replacement still deliver it, issue #79); `context` for unparseable bodies (openai-responses / google / bedrock / cursor). Explicit: always honored. `provider` transforms the provider wire payload (request-local, structurally immune to feedback re-entry); `context` rewrites the context event (legacy mode). |
